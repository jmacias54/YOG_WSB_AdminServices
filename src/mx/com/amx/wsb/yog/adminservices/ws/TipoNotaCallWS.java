 ...
 *
 */
public class TipoNotaCallWS {
	private static Logger logger = Logger.getLogger(TipoNotaCallWS.class);

	private RestTemplate restTemplate;
	private HttpHeaders headers = new HttpHeaders();
	private final Properties props = new Properties();

	public TipoNotaCallWS() {
		super();
	...
	public List<TipoNota> findAll() throws CallWSException {
		logger.debug("--- findAll --- [ TipoNotaCallWS ] --- ");
		...
		try {
			...
			String URL_WS = parametros.getUrl() + parametros.getCatalogos()  + parametros.getTipoNotaController();

			logger.debug("URL_WS: " + URL_WS);

			HttpEntity<String> entity = new HttpEntity<String>("Accept=application/json; charset=utf-8", headers);
			lista = Arrays.asList(restTemplate.postForObject(URL_WS, entity, TipoNota[].class));

		} catch (RestClientResponseException rre) {
			logger.error("RestClientResponseException findAll [ TipoNotaCallWS ]: " + rre.getResponseBodyAsString());
			logger.error("RestClientResponseException findAll [ TipoNotaCallWS ]: ", rre);
			throw new CallWSException(rre.getResponseBodyAsString());
		} catch (Exception e) {
			logger.error("Exception findAll  [ TipoNotaCallWS ]: ", e);
	...
	public TipoNota findById(String idTipoNota) throws CallWSException {
		logger.debug("--- findByIdTipoNota --- [ TipoNotaCallWS ] --- ");
		...
		try {
			...
			parametros = properties.obtenerPropiedades();
			String URL_WS = parametros.getUrl() + parametros.getCatalogos()  +parametros.getTipoNotaController() + idTipoNota + "/";
			logger.debug("URL_WS: " + URL_WS);

			HttpEntity<String> entity = new HttpEntity<String>("Accept=application/json; charset=utf-8", headers);
			return restTemplate.postForObject(URL_WS, entity, TipoNota.class);

		} catch (RestClientResponseException rre) {
			logger.error("RestClientResponseException findByIdTipoNota [ TipoNotaCallWS ]: "
					+ rre.getResponseBodyAsString());
			logger.error("RestClientResponseException findByIdTipoNota [ TipoNotaCallWS ]: ", rre);
			throw new CallWSException(rre.getResponseBodyAsString());
		} catch (Exception e) {
			logger.error("Exception findByIdTipoNota  [ TipoNotaCallWS ]: ", e);
			throw new CallWSException(e.getMessage());
		}
